...
// diverge. The build passes data.generated (a stable, committed date) so the
// render is deterministic and the drift gate never trips on a calendar boundary.
const homeRows = [...providers]
  .sort((a, b) => recScore(b) - recScore(a) || (a.name < b.name ? -1 : a.name > b.name ? 1 : 0))
...
// Emit the shared row renderer (htmlEsc / ICON / SLUG_RE / rowHtml) so the
// client explorer repaints with the exact same markup the server shipped.
const sharedRowsJs = `// AUTO-GENERATED by scripts/build.mjs from scripts/lib/rows.mjs — do not edit.
window.FLLM_ROWS = (function () {
  const { FLAG_PAIRS, freshnessStatus } = window.FLLM_RULES;
  const htmlEsc = ${rows.htmlEsc.toString()};
  const ICON = ${rows.ICON.toString()};
  const SLUG_RE = ${rows.SLUG_RE.toString()};
  const rowHtml = ${rows.explorerRowHtml.toString()};
  return { rowHtml };
})();
`;
writeFileSync(join(ROOT, 'site/shared-rows.js'), sharedRowsJs);
