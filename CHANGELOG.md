...
- **Paced re-verification — batch 5 (2026-08-14).** Rime now states "No credit card required" (`card_required` null→false) and has an internal inconsistency (the pricing table says ~800 free minutes while the FAQ/docs say 3,000 — recorded in the notes). Voyage AI's free bucket now lists `voyage-code-4` (launched 2026-08-13) instead of `voyage-code-3`, plus `voyage-4-large` and `voyage-multimodal-3`. Tencent Hunyuan noted the TokenHub migration. Contextual AI, Cartesia and LMNT confirmed unchanged. [#164](https://github.com/pacocartones/free-llm-api-hub/pull/164)
...
- **README Contributors section is now deterministic.** It renders from the committed `data/contributors.json` (refreshed via `scripts/update-contributors.mjs`) instead of being mined from `git log` inside the build — the old approach was self-referential and blocked external PRs at the drift gate (a contributor's own unmerged commit rewrote the README). Personal-email contributors also get a profile link via an optional `login` field.
...
- **Typhoon (SCB 10X) added** — the FAQ describes the API as "a research showcase and free to use"; Thai-language models incl. `typhoon-v2.1-12b-instruct` and `typhoon-ocr`, OpenAI-compatible at `api.opentyphoon.ai/v1`. Verified 2026-08-14. [#149](https://github.com/pacocartones/free-llm-api-hub/pull/149)
- **Typhoon — modalities extended to audio.** Re-verified 2026-08-14: the ASR docs page documents Typhoon ASR Real-Time on the hosted OpenAI-compatible API (`typhoon-asr-realtime`, 100 reqs/min), alongside the LLM and OCR catalog.

### Added
- **`/best` — the editorial top 10.** [`data/best.json`](data/best.json) holds the hand-picked ranking and a "why" per pick; the build validates every slug against `data/providers.json` (duplicate or unknown slugs fail the build) and renders the page with live verified data. Sitemap and robots flow through the normal pipeline.
- **Weekly model-catalog backfill.** `backfill.yml` (deleted 2026-08-14) refreshes `models_free` from the providers' live public `/models` endpoints every Monday (03:00 UTC) and opens a PR via `GITHUB_TOKEN` — never a direct push to `main`. It regenerates every derived artifact, runs the full test suite and the drift gate on the branch, then dispatches `verify.yml` so the PR's content gets the same check as a human PR.
- **Generated outputs are fingerprinted.** Every gitignored build output is gated by a sha256 `derived-fingerprints.json`, so a regenerated-but-uncommitted payload cannot silently diverge.

### Changed
- **Re-verification is local and on demand; model samples are scheduled.** `regenerate.yml`, `maintenance.yml`, `probe.yml` and `og-regenerate.yml` are deleted. Derived files are regenerated and committed in the same PR (`npm run build`, `npm run og`), and the weekly re-verification pass runs on demand via `scripts/reverify.mjs` (agentic Layer 3 of the [update playbook](docs/update-playbook.md)). `verify.yml` (the required "Dataset integrity" check) now fails a PR whose generated files are out of sync, and `badge-freshness.json` is excluded from the diff-gates because it is date-relative by design. The one scheduled Actions job is the model-catalog backfill above, which ships through a PR and never writes to `main` directly.
- **`fetch-models` guards its own registry.** A public extractor that vanishes from the `PUBLIC_REQUIRED` registry (the ollama-cloud incident) now fails CI instead of silently skipping the provider; the ollama-cloud extractor was restored.
- **Honest sorting.** The "What's free" column sorts by the `free_type` taxonomy (not prose text), and the Verified column sorts by `last_verified` date, not the boolean.
- **Deploys are verified live.** `pages.yml` now runs a `verify-live` job after each deploy: the committed sitemap must byte-match the deployed one (with retries for edge propagation) and every URL of the published sitemap must answer HTTP 200.
- **The site's freshness card reports the 90-day SLA, not a 7-day window.**
- **Site polish.** Navbar Home link removed (the logo is the home link); the quick-pick filters (Starter / OpenAI drop-in / Speech) are gone; the trust line above the table was dropped and the live verified badge moved into the hero; the Verified column stacks the check and the date, centered; the footer was trimmed (Updates moved to Project; Changelog, Guides & Collections and Notably NOT free removed) with a bigger, brighter brand block; the README header now shows the (animated) logo without the title text or the generated stats line; legal pages now use the project email admin@freellmapihub.com everywhere. The hero stat read "verified in last 7 days" and was permanently near zero at the project's real cadence; it now shows the oldest verification against the 90-day SLA (green < 60d, amber 60–90d, red > 90d), with the thresholds shared from `scripts/lib/rules.mjs`. The trust line no longer claims a weekly re-check that nothing enforces.
