...
- **CI:** `verify.yml` runs `validate` + `check-links` on every PR, plus an *informational* drift report. Derived files are regenerated locally — the author runs `npm run build` (and `npm run og` when needed) and commits the results in the same PR, so a change ships complete and nothing runs on a schedule. **Corollary: nothing date-relative may appear in committed generated files** (the "NEW" badge is client-side only; provider `/p/` pages are gitignored precisely so they can use the current date).
