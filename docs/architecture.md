...
- **CI:** `verify.yml` installs the locked development tools with lifecycle scripts disabled, then runs `validate` + `check-links` + `check-og` (OG images exist and match the dataset) + `check-history` (git-mined history) + the pipeline tests on every PR, plus a **blocking drift check**: it rebuilds the derived files and fails the PR if they are out of sync with the data. There is no regeneration bot — the author regenerates locally (`npm run build`, and `npm run og` when needed) and commits the results in the same PR, so a change ships complete — there is no scheduled job: re-verification, badge, probe and the `models_free` backfill (`npm run models`) all run locally and on-demand. **Corollary: nothing may be relative to the *current date*** in committed generated files — everything committed must be byte-stable for a given dataset (the "NEW" badge and the freshness colours are computed from `data.generated`, so they are deterministic and safe in committed files; provider `/p/` pages are gitignored precisely so they can use the current date).
...
- **`validate.mjs`** (`npm run validate`, part of `npm test`) — first validates `data/providers.json` against `data/schema.json` with the locked JSON Schema tooling (Draft 2020-12 + standard formats), then applies repository-specific integrity rules: unique slug/name, verified→(last_verified+docs_url), `added` freshness, programs.json shape and probe-report consistency. Exit 1 on any issue.
...
- **`check-og.mjs`** (CI) — every committed OG PNG must exist AND match the dataset: recomputes the fingerprints in `site/og/manifest.json` from `data/providers.json`. Pure Node, with no image-rendering dependency, so it checks the fingerprint without rendering images.
